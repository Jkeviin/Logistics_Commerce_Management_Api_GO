...
type SectionService interface {
	...
	Update(id int64, SectionAttributes domain.SectionAttributes) (sectionRes domain.Section, err error)

	// Delete is a method that eliminates a section from the repository
	Delete(id int64) (err error)

	// ReportProductBySection functions for sections with products
	ReportProductBySection(idSection int64) ([]domain.SectionWithProductCount, error)
}
